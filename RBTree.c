...
inline void left_rotate(struct list* x);
inline void right_rotate(struct list* x);
inline void tree_print(struct list* x);
inline void red_black_insert(int key);
inline void red_black_insert_fixup(struct list* z);
inline struct list* tree_search(int key);
inline struct list* tree_minimum(struct list* x);
inline void red_black_transplant(struct list* u, struct list* v);
inline void red_black_delete(struct list* z);
inline void red_black_delete_fixup(struct list* x);


/* Print tree keys by inorder tree walk */

void tree_print(struct list* x) {
	if (x != NILL) {
		tree_print(x->left);
		printf("%d\t", x->key);
		tree_print(x->right);
...
struct list* tree_search(int key) {
	struct list* x;
...
struct list* tree_minimum(struct list* x) {
	while (x->left != NILL) {
		x = x->left;
	}
	return x;
...
void red_black_insert_fixup(struct list* z) {
...
void left_rotate(struct list* x) {
	struct list* y;
...
void right_rotate(struct list* x) {
	struct list* y;
...
void red_black_delete(struct list* z) {
	struct list* y, * x;
	int yOriginalColor;
...
void red_black_delete_fixup(struct list* x) {
	struct list* w;
...
void red_black_transplant(struct list* u, struct list* v) {
	if (u->parent == NILL) {
		ROOT = v;
	}
